...
abstract class TestDataGeneratorBase {
    ...
    protected open fun writeTestData(
        sourceHtml: String,
        article: Article,
    ...
    protected open fun getTestCaseFolder(
        testFolderName: String,
        testCaseName: String,
    ): File {
    ...
    protected open fun writeFile(
        ...
        val writer =
            OutputStreamWriter(
                FileOutputStream(File(testCaseFolder, fileName)),
    ...
    protected open fun getPathToTestResourcesFolder(): File {
        val testPageResourceUrl =
            this.javaClass.classLoader.getResource(
                "logback-test.xml",
            ) // get url to any test resource that's for sure there
